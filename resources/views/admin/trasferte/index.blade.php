                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @forelse ($trasferte as $trasferta)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ $trasferta->posti_disponibili }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <form method="POST" action="{{ route('admin.trasferte.destroy', $trasferta) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400" onclick="return confirm('Sei sicuro di voler eliminare questa trasferta?')" title="Elimina">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
